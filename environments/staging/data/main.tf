...
module "shared" {
  ...
  purpose     = "Staging Aurora PostgreSQL, ElastiCache Redis, and Secrets Manager — DR drill target (cloud-architecture-blueprint.md Section 12)"
  retention   = "7-days"
}
...
module "aurora" {
  ...
  serverless          = true
  reader_count        = 1 # staging — exercises the reader-endpoint/PgBouncer read-pool path before production
...
module "elasticache" {
  ...
  num_shards         = 1
  replicas_per_shard = 1 # staging — "1 shard, 1 primary + 1 replica" (cloud-architecture-blueprint.md Section 6)
